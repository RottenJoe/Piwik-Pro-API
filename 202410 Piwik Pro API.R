...
for (i in names(queries)) {
  ...
  #Unlist & DF
  df <- as.data.frame(sapply(result, unlist_and_replace_null))
...
##Separate
test <- df %>% separate(unlist(website_name), sep = ',', into = c('website_name', 'website_id'))

##Unnest wider
df %>% unnest_wider(website_name, names_sep = '_', simplify = TRUE)
